...
setInterval(()=>{if($('#page-trade').classList.contains('active')){$('#trade-price').textContent=window.__publicQuoteFresh?.(currentMarket)===false?'--':fmt(currentMarket.price,decimals(currentMarket.price));$('#trade-high').textContent=fmt(currentMarket.high,decimals(currentMarket.high));$('#trade-low').textContent=fmt(currentMarket.low,decimals(currentMarket.low));{const chartBadge=$('#chart-price-badge');if(chartBadge)chartBadge.textContent=fmt(currentMarket.price,decimals(currentMarket.price))}}updateBalances()},1000);
...
/* ===== market-detail-ui.js ===== */
(()=>{
  const page=document.getElementById('page-markets'),list=document.getElementById('market-list'),panel=page?.querySelector('.market-panel');
  ...
  const descriptions={
    ...
    RENDER:'Render is a token used in a distributed GPU rendering network connecting computing providers with creators.',
  ...
  const detail=document.createElement('section');detail.className='panel market-detail-card';detail.hidden=true;
  detail.innerHTML='<div class="market-detail-top"><div class="market-detail-identity"><span class="coin-icon small market-detail-icon"></span><div><span class="eyebrow">MARKET DETAILS</span><h2 class="market-detail-symbol">—</h2><p class="market-detail-name">—</p></div></div><button type="button" class="market-detail-close" aria-label="Close market details">×</button></div><div class="market-detail-stats"><div><span>Last Price</span><strong class="market-detail-price">—</strong></div><div><span>24h Change</span><strong class="market-detail-change">—</strong></div><div><span>24h High</span><strong class="market-detail-high">—</strong></div><div><span>24h Low</span><strong class="market-detail-low">—</strong></div></div><p class="market-detail-description"></p><div class="market-detail-chart-head"><strong>Price Trend</strong><div class="market-detail-periods"><button type="button" data-detail-period="1H">1H</button><button type="button" data-detail-period="24H" class="active">24H</button><button type="button" data-detail-period="7D">7D</button><button type="button" data-detail-period="30D">30D</button></div></div><div class="market-detail-chart-wrap"><canvas class="market-detail-chart" aria-label="Market price trend"></canvas><div class="market-detail-chart-status">Select a market to load its trend.</div></div><div class="market-detail-actions"><button type="button" class="submit-trade market-detail-trade">Trade this market</button></div>';
  panel.appendChild(detail);
  const canvas=detail.querySelector('.market-detail-chart'),ctx=canvas.getContext('2d'),status=detail.querySelector('.market-detail-chart-status'),periodWrap=detail.querySelector('.market-detail-periods');
  let active=null,period='24H',token=0,rows=[];
  function pf(v){const n=Number(v);if(!Number.isFinite(n)||n<=0)return'—';return n.toLocaleString('en-US',{minimumFractionDigits:n>=1?2:4,maximumFractionDigits:n>=1000?2:n>=1?4:8})}
  function paint(){if(!active)return;const code=String(active.symbol||'').split('/')[0].toUpperCase();detail.querySelector('.market-detail-icon').innerHTML=typeof tradeIconHTML==='function'?tradeIconHTML(active):code;detail.querySelector('.market-detail-symbol').textContent=active.symbol||code;detail.querySelector('.market-detail-name').textContent=active.name||code;detail.querySelector('.market-detail-description').textContent=descriptions[code]||((active.name||code)+' is a digital asset available on this market.');detail.querySelector('.market-detail-price').textContent=pf(active.price);const ch=Number(active.change||0),ce=detail.querySelector('.market-detail-change');ce.textContent=(ch>=0?'+':'')+ch.toFixed(2)+'%';ce.classList.toggle('positive',ch>=0);ce.classList.toggle('negative',ch<0);detail.querySelector('.market-detail-high').textContent=pf(active.high);detail.querySelector('.market-detail-low').textContent=pf(active.low)}
  function size(){const b=canvas.getBoundingClientRect();if(b.width<2||b.height<2)return null;const d=Math.min(devicePixelRatio||1,2);canvas.width=Math.round(b.width*d);canvas.height=Math.round(b.height*d);ctx.setTransform(d,0,0,d,0,0);return{w:b.width,h:b.height}}
  function draw(){const s=size();if(!s)return;const w=s.w,h=s.h;ctx.clearRect(0,0,w,h);if(rows.length<2){status.hidden=false;return}status.hidden=true;const vals=rows.map(r=>Number(r.close)).filter(Number.isFinite);let lo=Math.min(...vals),hi=Math.max(...vals),pad=(hi-lo||Math.abs(hi)*.01||1)*.12;lo-=pad;hi+=pad;const L=10,R=10,T=12,B=22,W=w-L-R,H=h-T-B,x=i=>L+W*i/(rows.length-1),y=v=>T+(hi-v)/(hi-lo)*H;ctx.strokeStyle='rgba(126,163,196,.14)';ctx.lineWidth=1;for(let i=0;i<4;i++){const yy=T+H*i/3;ctx.beginPath();ctx.moveTo(L,yy);ctx.lineTo(L+W,yy);ctx.stroke()}ctx.beginPath();rows.forEach((r,i)=>{const xx=x(i),yy=y(Number(r.close));i?ctx.lineTo(xx,yy):ctx.moveTo(xx,yy)});ctx.strokeStyle='#36c5f0';ctx.lineWidth=2;ctx.stroke();const g=ctx.createLinearGradient(0,T,0,T+H);g.addColorStop(0,'rgba(54,197,240,.22)');g.addColorStop(1,'rgba(54,197,240,0)');ctx.lineTo(L+W,T+H);ctx.lineTo(L,T+H);ctx.closePath();ctx.fillStyle=g;ctx.fill();ctx.fillStyle='#7894ad';ctx.font='10px Inter,Arial';const f=new Date(Number(rows[0].time)*1000),l=new Date(Number(rows[rows.length-1].time)*1000),lab=d=>d.toLocaleString('en-US',{month:'short',day:'numeric'}),a=lab(f),z=lab(l);ctx.fillText(a,L,h-5);ctx.fillText(z,Math.max(L,w-R-ctx.measureText(z).width),h-5)}
  async function load(){if(!active)return;const id=++token,code=String(active.symbol||'').split('/')[0].toUpperCase();rows=[];status.hidden=false;status.textContent='Loading real market trend…';draw();try{const api=window.DAppsPlatformConfig?.apiBase;if(!api)throw Error('api');const r=await fetch(api+'/api/market/chart?symbol='+encodeURIComponent(code)+'&period='+encodeURIComponent(period),{cache:'no-store',headers:{Accept:'application/json'}});if(!r.ok)throw Error(String(r.status));const d=await r.json();if(id!==token)return;rows=(Array.isArray(d.candles)?d.candles:[]).filter(x=>Number.isFinite(Number(x.time))&&Number.isFinite(Number(x.close)));status.textContent=rows.length?'':'Trend unavailable';draw()}catch(_){if(id===token){rows=[];status.hidden=false;status.textContent='Trend temporarily unavailable';draw()}}}
