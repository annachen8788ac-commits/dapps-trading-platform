...
const DEMO_CLOSE_GRACE_MS=15000;
function validDemoSessionId(value){const id=String(value||'').trim();return /^[A-Za-z0-9-]{12,80}$/.test(id)?id:null}
function pruneDemoSessions(){const now=Date.now();for(const[id,s]of demoSessions)if(Number(s.pendingLeaveAt||0)>0&&Number(s.pendingLeaveAt)<=now)demoSessions.delete(id)}
...
app.post('/api/demo/presence',async(req,res)=>{const sessionId=validDemoSessionId(req.body?.sessionId);if(!sessionId)return res.status(400).json({error:'Invalid simulation session'});pruneDemoSessions();const now=Date.now(),existing=demoSessions.get(sessionId),s=existing||{sessionId,label:'SIM-'+sessionId.replace(/[^A-Za-z0-9]/g,'').slice(0,8).toUpperCase(),createdAt:now,control:'auto',forceQueue:[]};if(!existing)await pool.query(`INSERT INTO demo_session_notifications(session_id,label) VALUES($1,$2) ON CONFLICT(session_id) DO NOTHING`,[sessionId,s.label]);s.pendingLeaveAt=null;s.lastSeen=now;s.page=['trade','pledge'].includes(String(req.body?.page))?String(req.body.page):'trade';for(const key of ['balance','activeTrades','pledged','pledgeCount']){const n=Number(req.body?.[key]);if(Number.isFinite(n)&&n>=0)s[key]=Math.min(n,1e12)}demoSessions.set(sessionId,s);res.json({ok:true,control:s.control,forceQueue:s.forceQueue})});
app.post('/api/demo/leave',(req,res)=>{const sessionId=validDemoSessionId(req.body?.sessionId||req.query?.sessionId),mode=String(req.body?.mode||req.query?.mode||'explicit');if(sessionId){const s=demoSessions.get(sessionId);if(mode==='unload'&&s)s.pendingLeaveAt=Date.now()+DEMO_CLOSE_GRACE_MS;else demoSessions.delete(sessionId)}res.json({ok:true})});
