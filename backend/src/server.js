...
async function initializeDatabase(){
  ...
  for(let i=0;i<markets.length;i++)await pool.query(`INSERT INTO market_settings(symbol,display_name,sort_order) VALUES($1,$1,$2) ON CONFLICT(symbol) DO NOTHING`,[markets[i],i+1]);
...
app.get('/api/market/history',async(req,res)=>{
  ...
  const key='history:'+id,hit=marketCache.get(key);if(hit&&Date.now()-hit.at<60000)return res.json(hit.data);
  try{const upstream=await fetch(`https://api.coingecko.com/api/v3/coins/${id}/market_chart?vs_currency=usd&days=1`,{signal:AbortSignal.timeout(12000)});if(!upstream.ok)throw Error(String(upstream.status));const data=await upstream.json();marketCache.set(key,{at:Date.now(),data});res.json(data)}catch(e){if(hit)return res.json(hit.data);res.status(503).json({error:'Market history unavailable'})}
});
