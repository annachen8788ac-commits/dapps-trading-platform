...
export function registerSupportChatRoutes(app,{pool,auth,adminAuth,audit}){
  async function convo(userId){let q=await pool.query(`SELECT id,status FROM support_conversations WHERE user_id=$1`,[userId]);if(q.rows[0])return q.rows[0];q=await pool.query(`INSERT INTO support_conversations(user_id) VALUES($1) RETURNING id,status`,[userId]);return q.rows[0]}
  app.get('/api/support/chat',auth,async(req,res)=>{try{const c=await convo(req.auth.sub);const [u,m]=await Promise.all([pool.query(`SELECT public_id,display_name FROM users WHERE id=$1`,[req.auth.sub]),pool.query(`SELECT id,sender_type,message,created_at FROM support_messages WHERE conversation_id=$1 ORDER BY id ASC LIMIT 500`,[c.id])]);res.json({conversation:{id:c.id,status:c.status},user:{publicId:u.rows[0]?.public_id,displayName:u.rows[0]?.display_name},agent:{displayName:'DApps Support'},messages:m.rows.map(x=>({id:x.id,sender:x.sender_type,message:x.message,createdAt:x.created_at}))})}catch(e){console.error(e);res.status(500).json({error:'Unable to load support chat'})}});
  app.post('/api/support/chat/messages',auth,async(req,res)=>{const message=clean(req.body?.message);if(!message)return res.status(400).json({error:'Message is required'});try{const c=await convo(req.auth.sub);const q=await pool.query(`INSERT INTO support_messages(conversation_id,sender_type,sender_id,message) VALUES($1,'user',$2,$3) RETURNING id,created_at`,[c.id,req.auth.sub,message]);await pool.query(`UPDATE support_conversations SET status='open',updated_at=NOW() WHERE id=$1`,[c.id]);res.status(201).json({message:{id:q.rows[0].id,sender:'user',message,createdAt:q.rows[0].created_at}})}catch(e){console.error(e);res.status(500).json({error:'Unable to send message'})}});
