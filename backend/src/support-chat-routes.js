...
export function registerSupportChatRoutes(app,{pool,auth,adminAuth,audit}){
  ...
  app.get('/api/admin/support/chats',adminAuth,async(req,res)=>{try{const q=await pool.query(`SELECT c.id,c.status,c.updated_at,u.public_id,u.display_name,u.identifier,last_all.message last_message,last_all.created_at last_message_at,last_user.created_at last_user_message_at,(last_user.created_at IS NOT NULL AND (r.read_at IS NULL OR last_user.created_at>r.read_at)) unread FROM support_conversations c JOIN users u ON u.id=c.user_id LEFT JOIN LATERAL (SELECT message,created_at FROM support_messages m WHERE m.conversation_id=c.id ORDER BY m.id DESC LIMIT 1) last_all ON TRUE LEFT JOIN LATERAL (SELECT created_at FROM support_messages m WHERE m.conversation_id=c.id AND m.sender_type='user' ORDER BY m.id DESC LIMIT 1) last_user ON TRUE LEFT JOIN admin_notification_reads r ON r.admin_id=$1 AND r.source_type='support_chat' AND r.source_id=c.id::text ORDER BY (last_user.created_at IS NOT NULL AND (r.read_at IS NULL OR last_user.created_at>r.read_at)) DESC,COALESCE(last_all.created_at,c.updated_at) DESC LIMIT 300`,[req.admin.id]);res.json({chats:q.rows.map(x=>({id:x.id,status:x.status,updatedAt:x.updated_at,lastMessage:x.last_message,lastMessageAt:x.last_message_at,unread:Boolean(x.unread),user:{publicId:x.public_id,displayName:x.display_name,identifier:x.identifier}}))})}catch(e){console.error(e);res.status(500).json({error:'Unable to load chats'})}});
