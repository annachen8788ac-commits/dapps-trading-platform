(() => {
  ...
  style.textContent = `
    ...
    #page-markets .market-row{cursor:default}
    ...
    .pair-menu{position:absolute;left:0;top:42px;width:310px;max-height:390px;overflow:hidden;background:#111520;border:1px solid #30384c;border-radius:12px;box-shadow:0 18px 50px rgba(0,0,0,.45);z-index:60;display:none}
    ...
    .pair-search:focus{border-color:#4d7cff}.pair-list{max-height:330px;overflow:auto;padding:6px}
    ...
    .pair-item:hover,.pair-item.active{background:#1b2232}.pair-item .left{display:flex;align-items:center;gap:9px}.pair-item .mini-icon{width:26px;height:26px;border-radius:50%;display:grid;place-items:center;color:#fff;font-size:11px;font-weight:800}.pair-item strong{font-size:12px}.pair-item small{display:block;color:#7f8a9f;margin-top:2px}.pair-item .price{text-align:right}.pair-item .price b{display:block;font-size:12px}.pair-item .price span{font-size:10px}
    @media(max-width:720px){#page-markets .market-table-head,#page-markets .market-row{grid-template-columns:1.45fr 1fr .85fr}#page-markets .market-table-head span:nth-child(4),#page-markets .market-row>*:nth-child(4){display:none}.trade-pair-selector{margin-left:0}.pair-select-btn{padding:6px 8px}.pair-menu{position:fixed;left:12px;right:12px;bottom:92px;top:auto;width:auto;max-height:58vh;border-radius:16px}.pair-list{max-height:48vh}}
  `;
  ...
  selector.innerHTML = `
    <button class="pair-select-btn" id="pair-select-btn" type="button"><span>Change Market</span><span class="chev">▼</span></button>
    <div class="pair-menu" id="pair-menu">
      <div class="pair-search-wrap"><input class="pair-search" id="pair-search" type="search" placeholder="Search market" autocomplete="off"></div>
  ...
  function renderPairList(query=''){
    ...
    list.innerHTML='';
    markets.filter(m=>!q||m.symbol.toLowerCase().includes(q)||m.name.toLowerCase().includes(q)).forEach(m=>{
