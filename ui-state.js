(() => {
  ...
  function syncSimulationBalance(){
    ...
    const key='dapps:demoBalance:v3:'+demoId;
    let saved=null;try{saved=sessionStorage.getItem(key)}catch{}
    const n=saved===null?50000:Number(saved);
    balance=Number.isFinite(n)?n:50000;
    ...
    const note=document.querySelector('.asset-main small');if(note){note.textContent='Simulation balance · 50,000 USDT starting funds';note.className='positive'}
  }

  const baseNavigate=navigate;
  navigate=function(name){if(!validPages.has(name))name='home';safeSet(STORAGE.page,name);if(location.hash!==`#${name}`)history.replaceState(null,'',`${location.pathname}${location.search}#${name}`);return baseNavigate(name)};
