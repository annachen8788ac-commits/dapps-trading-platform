(()=>{
  ...
  async function loadCatalog(){
    ...
    const all=[];
    try{
      for(const p of pages){const r=await fetch(`${backend}/api/market/catalog?page=${p}`,{cache:'no-store'});if(!r.ok)throw new Error('market data unavailable');const d=await r.json();if(Array.isArray(d))all.push(...d)}
