(()=>{const nativeAlert=window.alert.bind(window);let pending=0,lastClicked=null,lastClickedAt=0,lastAlertAt=0,progressTimer=null;
...
window.fetch=async(input,init={})=>{const url=typeof input==='string'?input:String(input?.url||''),method=String(init?.method||input?.method||'GET').toUpperCase(),adminReq=url.includes('/admin-api'),mutation=adminReq&&!['GET','HEAD'].includes(method);let btn=null,original='';
...
try{const r=await rawFetch(input,init);if(mutation){if(!r.ok){let data={};try{data=await r.clone().json()}catch{}notify(data?.error||('请求失败（'+r.status+'）'),'error')}}return r}
catch(err){if(mutation)notify(err?.message||'网络请求失败','error');throw err}
finally{if(btn){btn.classList.remove('is-processing');btn.disabled=false;btn.textContent=original}if(mutation)progress(false)}};
