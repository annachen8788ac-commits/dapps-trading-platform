...
function sessionToken(req){
  const sid=cookieMap(req)[COOKIE_NAME];
...
async function backend(path, options={}){
  const r=await fetch(`${BACKEND_URL}${path}`,options);
  const text=await r.text();
  let data; try{ data=text ? JSON.parse(text) : {}; }catch{ data={ error:'Invalid backend response' }; }
  return { status:r.status, data };
}
function requireSession(req,res,next){
  if(!sessionToken(req)) return res.status(401).json({ error:'Admin authentication required' });
  next();
}
...
app.all('/admin-api/*', requireSession, async (req,res)=>{
  ...
  try{
    const result=await backend(path,{ method:req.method,headers,body });
