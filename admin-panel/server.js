...
app.get('/',async(req,res,next)=>{try{let html=await localizedHtml('index.html');
  html=html.replace(`<button data-section="tickets">Support</button>`,`<button class="external" onclick="location.href='/support-chat'">在线客服</button>`);
  if(!html.includes("location.href='/trades'")){html=html.replace(`<button class="external" onclick="location.href='/kyc'">KYC Management</button>`,`<button class="external" onclick="location.href='/kyc'">KYC Management</button><button class="external" onclick="location.href='/trades'">交易控制</button>`);html=html.replace(`<a href="/kyc"><b>KYC Management →</b><span class="muted">Review identity applications and approve or reject submissions</span></a>`,`<a href="/kyc"><b>KYC Management →</b><span class="muted">Review identity applications and approve or reject submissions</span></a><a href="/trades"><b>交易控制 →</b><span class="muted">查看全部账户交易，并在到期前设置自动 / 赢 / 输</span></a>`)}
  if(!html.includes('href="/support-chat"'))html=html.replace('</div></section>\n<section id="users"',`<a href="/support-chat"><b>在线客服 →</b><span class="muted">查看客户实时会话并直接回复消息</span></a></div></section>\n<section id="users"`);
