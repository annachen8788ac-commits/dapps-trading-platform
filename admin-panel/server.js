...
async function localizedHtml(name){const file=new URL('./public/'+name,import.meta.url).pathname;let html=await readFile(file,'utf8');html=html.replace(/<html lang="en">/,'<html lang="zh-CN">');html=html.replace('</head>','<link rel="icon" type="image/png" sizes="64x64" href="/favicon.png"><link rel="stylesheet" href="/admin-brand.css?v=20260925-3"></head>');if(!html.includes('/admin-ui.js'))html=html.replace('</body>','<script src="/admin-ui.js?v=20260925-1"></script></body>');if(!html.includes('/admin-zh.js'))html=html.replace('</body>','<script src="/admin-zh.js"></script></body>');if(!html.includes('/admin-notifications.js'))html=html.replace('</body>','<script src="/admin-notifications.js"></script></body>');return html}
