...
function sessionToken(req){return cookieMap(req)[COOKIE_NAME]||null}
function setSession(res,token){res.setHeader('Set-Cookie',`${COOKIE_NAME}=${encodeURIComponent(token)}; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=${COOKIE_MAX_AGE}`)}
function clearSession(req,res){res.setHeader('Set-Cookie',`${COOKIE_NAME}=; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=0`)}
...
app.get('/session/me',requireSession,async(req,res)=>{try{const token=sessionToken(req),result=await backend('/api/admin/me',{headers:{authorization:`Bearer ${token}`}});if(result.status===401)clearSession(req,res);else if(result.status<400)setSession(res,token);res.status(result.status).json(result.data)}catch(e){console.error(e);res.status(502).json({error:'Admin backend unavailable'})}});
app.all('/admin-api/*',requireSession,async(req,res)=>{const token=sessionToken(req),path=req.originalUrl.replace(/^\/admin-api/,'/api/admin'),headers={authorization:`Bearer ${token}`};let body;if(!['GET','HEAD'].includes(req.method)){headers['content-type']='application/json';body=JSON.stringify(req.body||{})}try{const result=await backend(path,{method:req.method,headers,body});if(result.status===401)clearSession(req,res);else if(result.status<400)setSession(res,token);res.set('Cache-Control','no-store');res.status(result.status).json(result.data)}catch(e){console.error(e);res.status(502).json({error:'Admin backend unavailable'})}});
async function localizedHtml(name){const file=new URL('./public/'+name,import.meta.url).pathname;let html=await readFile(file,'utf8');html=html.replace(/<html lang="en">/,'<html lang="zh-CN">');html=html.replace('</head>','<link rel="icon" type="image/png" sizes="64x64" href="/favicon.png"><link rel="stylesheet" href="/admin-brand.css?v=20260925-7"></head>');if(!html.includes('/admin-ui.js'))html=html.replace('</body>','<script src="/admin-ui.js?v=20260925-6"></script></body>');if(!html.includes('/admin-zh.js'))html=html.replace('</body>','<script src="/admin-zh.js"></script></body>');if(!html.includes('/admin-notifications.js'))html=html.replace('</body>','<script src="/admin-notifications.js?v=20260925-3"></script></body>');return html}
...
for(const [route,file] of [['/wallet','wallet.html'],['/kyc','kyc.html'],['/trades','trades.html'],['/support-chat','support-chat.html']])app.get(route,async(req,res,next)=>{try{res.type('html').send(await localizedHtml(file))}catch(e){next(e)}});
