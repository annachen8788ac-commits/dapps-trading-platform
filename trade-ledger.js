(() => {
  ...
  const style=document.createElement('style');style.textContent=`.positions-table-head,.position-row{grid-template-columns:1.05fr .75fr 1fr 1fr 1.15fr .8fr .85fr;align-items:center}.status-pill{display:inline-flex;padding:5px 8px;border-radius:999px;font-weight:700;font-size:10px}.status-pill.active{background:rgba(77,124,255,.12);color:#8facff}.status-pill.won{background:rgba(40,199,124,.12);color:#62dca0}.status-pill.lost{background:rgba(255,91,103,.12);color:#ff8b94}.trade-time{color:#a8b2c7;white-space:nowrap}.countdown{font-variant-numeric:tabular-nums;font-weight:700}@media(max-width:720px){.positions-panel{overflow-x:auto}.positions-table-head,.position-row{display:grid!important;grid-template-columns:120px 90px 110px 110px 155px 90px 85px;min-width:760px;padding:12px 14px}}`;document.head.appendChild(style);
  ...
  let backendTrades=[],mode='active',settling=new Set();
  ...
  function renderHistory(){const rows=backendTrades.filter(t=>t.result!=='pending');head.innerHTML='<span>Pair</span><span>Result</span><span>Entry</span><span>Exit</span><span>Amount</span><span>Opened</span><span>Closed</span>';if(!rows.length){list.className='positions-list empty-state';list.textContent='No completed trades yet.';return}list.className='positions-list';list.innerHTML=rows.map(t=>`<div class="position-row"><strong>${t.symbol}</strong><span><i class="status-pill ${t.result==='won'?'won':'lost'}">${t.result==='won'?'Won':'Lost'}</i></span><strong>${fmt(t.entryPrice,decimals(t.entryPrice))}</strong><strong>${fmt(t.exitPrice||0,decimals(t.exitPrice||t.entryPrice))}</strong><span>${fmt(t.amount)} USDT</span><span class="trade-time">${fmtTime(t.openedAt)}</span><span class="trade-time">${fmtTime(t.settledAt)}</span></div>`).join('')}
